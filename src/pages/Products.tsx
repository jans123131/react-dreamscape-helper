...
const Products = ({ selectedCategory, selectedSubcategory, selectedProductId }: ProductsProps) => {
  ...
  if (selectedProductId) {
    ...
    // If a specific product is found by ID, automatically display its detail view
    if (filteredProducts.length === 1 && selectedProductIdState === null) {
  ...
  // Otherwise, filter by category/subcategory
    ...
    if (selectedCategory.includes(',')) {
      filteredProducts = products.filter(product => 
        selectedCategory.split(',').some(cat => product.category === cat)
      );
    } else {
