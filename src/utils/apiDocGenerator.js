...
class ApiDocGenerator {
  ...
  generateClassDiagram() {
    const apiConfig = require('../config/apiConfig').API_CONFIG;
    const entities = apiConfig.entities || [];
    const diagramFilePath = path.join(this.docsDir, 'diagram_de_class.html');
...
`;
    ...
    entities.forEach(entity => {
        ...
        entity.fields.forEach(field => {
            const isPrimary = field.isPrimary ? 'primary-key' : '';
            const fieldDescription = field.description || 
                (field.isPrimary ? 'Clé primaire' : 
                    (field.reference ? `Référence à ${field.reference}` : ''));
            
            htmlContent += `
                        <tr>
                            <td class="${isPrimary}">${field.name}</td>
                            <td>${field.type}</td>
        ...
        // Add relationships section if entity has relations
        if (entity.relations && entity.relations.length > 0) {
            ...
            entity.relations.forEach(relation => {
                ...
                if (relation.type === 'hasMany') {
                    relationshipText = `Possède plusieurs ${relation.entity} via ${relation.via}`;
                } else if (relation.type === 'belongsTo') {
                    relationshipText = `Appartient à ${relation.entity} via ${relation.via}`;
                } else if (relation.type === 'hasOne') {
                    relationshipText = `Possède un ${relation.entity} via ${relation.via}`;
                }
    ...
    htmlContent += `
        ...
        <footer>
            <p>JenCity Diagramme de Classes - Généré le ${new Date().toLocaleDateString('fr-FR')}</p>
            <p>Version 1.0</p>
        </footer>
    </div>
    ...
    fs.writeFileSync(diagramFilePath, htmlContent);
    console.log(`Class diagram generated at: ${diagramFilePath}`);
  }
