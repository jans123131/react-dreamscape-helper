...
// Auth stack - with simplified password reset flow
const AuthStack = () => {
  return (
    <Stack.Navigator 
  ...
  return (
    <RootStack.Navigator screenOptions={{ headerShown: false }}>
      {!isAuthenticated ? (
        ...
        <RootStack.Screen name={STACKS.ADMIN} component={AdminStack} />
      ) : user?.role === 'provider' || user?.role === 'prestataire' ? (
        <RootStack.Screen name={STACKS.PROVIDER} component={ProviderStack} />
      ) : (
        <RootStack.Screen name={STACKS.USER} component={UserStack} />
      )}
    </RootStack.Navigator>
  );
};
